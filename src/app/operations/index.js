...
import {
  ArrowUpFromLine, Banknote, Bell, Clock, CreditCard, FileDigit, FileText, Globe,
  History, Languages, Menu, MessageSquare, Monitor, MonitorPlay, Package, Percent,
  PlusCircle, Printer, QrCode, Receipt, RefreshCw, Sun, Tags, Truck, User, Users, Wallet,
...
const OPERATIONS_OPTIONS = [
  ...
  { id: "extra-history", label: "Extra Information History", icon: FileText, route: null },
  ...
  { id: "language-profiles", label: "Language Profiles", icon: Languages, route: null },
  { id: "billing-user", label: "Billing User Profile", icon: User, route: "/staff" },
  { id: "day-end", label: "Day End", icon: Sun, route: "/day-end" },
  { id: "day-end-history", label: "Day End History", icon: History, route: null },
...
const CONFIGURATION_OPTIONS = [
  ...
  { id: "online-order", label: "Online Order Configuration", icon: Globe, route: "/online-order-config" },
  { id: "service-renewal", label: "Service Renewal", icon: Clock, route: null },
];

export default function OperationsPage() {
  const { isDesktop, isTablet, isMiniTab, isMobile, isLaptop, isWebDesktop, width } = useResponsive();
  ...
  const numColumns = isDesktop ? 8 : isLaptop ? 6 : isTablet ? 5 : isMiniTab ? 4 : 3;
  const gap = ThemeSpacing.lg;

  const handlePress = (id, route, label) => {
    if (id === "discount") { setDiscountModalVisible(true); }
    else if (id === "service-renewal") { setRenewalModalVisible(true); }
    else if (id === "billing-screen") { setBillingModalVisible(true); }
    else if (id === "print") { setPrintModalVisible(true); }
    else if (id === "tax") { setTaxModalVisible(true); }
    else if (route) { router.push(route); }
    else { showAlert("Coming Soon", `${label} is currently under construction.`); }
  };

  const isUnderConstruction = (id, route) => {
    const specialIds = ["discount", "service-renewal", "billing-screen", "print", "tax"];
    ...
    <View style={[styles.gridContainer, { gap }]}>
      {options.map((item) => {
        ...
        return (
          <TouchableOpacity 
            key={item.id} 
            style={[styles.tile, { width: itemWidth }, muted && { opacity: 0.5 }]} 
            onPress={() => handlePress(item.id, item.route, item.label)} 
            activeOpacity={0.7}
          >
            <Icon size={36} color={ThemeColors.textPrimary} strokeWidth={1.2} />
            <Text weight="medium" style={styles.tileText} numberOfLines={2}>{item.label}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
    ...
    <View style={styles.root}>
      <SafeAreaView edges={["top"]} style={styles.headerSafe}>
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            {!isWebDesktop && (
              <TouchableOpacity onPress={() => navigation.dispatch({ type: "TOGGLE_DRAWER" })} style={styles.menuBtn}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.section}>
          <Text weight="bold" style={styles.sectionTitle}>Operations</Text>
          {renderGrid(OPERATIONS_OPTIONS)}
        </View>
        <View style={styles.section}>
          <Text weight="bold" style={styles.sectionTitle}>Set the configuration for your restaurant</Text>
          {renderGrid(CONFIGURATION_OPTIONS)}
        </View>
      </ScrollView>

      <DiscountConfigModal visible={discountModalVisible} onClose={() => setDiscountModalVisible(false)} />
      <ServiceRenewalModal visible={renewalModalVisible} onClose={() => setRenewalModalVisible(false)} />
      <BillingConfigModal visible={billingModalVisible} onClose={() => setBillingModalVisible(false)} />
      <PrintConfigModal visible={printModalVisible} onClose={() => setPrintModalVisible(false)} />
      <TaxConfigModal visible={taxModalVisible} onClose={() => setTaxModalVisible(false)} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  headerSafe: { backgroundColor: ThemeColors.surface, borderBottomWidth: 1, borderColor: ThemeColors.border, zIndex: 100, elevation: 100 },
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: ThemeSpacing.xxl, paddingVertical: ThemeSpacing.md },
  ...
  headerRight: { flexDirection: "row", alignItems: "center", gap: ThemeSpacing.lg },
  notifBtn: { position: "relative", padding: 4 },
  notifDot: { position: "absolute", top: 4, right: 4, width: 8, height: 8, borderRadius: 4, backgroundColor: ThemeColors.red, borderWidth: 1.5, borderColor: ThemeColors.surface },
  scrollContent: { flexGrow: 1, padding: ThemeSpacing.lg },
  section: { marginBottom: ThemeSpacing.lg },
  sectionTitle: { fontSize: 18, color: ThemeColors.textPrimary, marginBottom: ThemeSpacing.lg },
  ...
  tileText: { marginTop: ThemeSpacing.lg, fontSize: 13, color: ThemeColors.textSecondary, textAlign: "center" },
});
