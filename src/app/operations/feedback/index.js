...
const MOCK_FEEDBACK = [
  {
    ...
    value: 4,
    comment:
      "Excellent food! Butter chicken was amazing. Will definitely come again.",
    table: "A3",
  },
  {
    id: "FB-2",
    ...
    ambience: 4,
    value: 3,
    comment: "Good food, slightly overpriced for portions.",
    table: "A1",
  },
  {
    id: "FB-3",
    ...
    value: 5,
    comment:
      "Best biryani in town! Service could be a bit faster during peak hours.",
    table: "B2",
  },
  {
    id: "FB-4",
    ...
    value: 3,
    comment:
      "Food was okay. Waited too long for our order. Staff seemed overwhelmed.",
    table: "A5",
  },
  {
    id: "FB-5",
    ...
    ambience: 3,
    value: 4,
    comment: "Delicious thali! Great value for money.",
    table: "A6",
  },
  {
    id: "FB-6",
    ...
    ambience: 5,
    value: 5,
    comment: "Perfect dinner experience. Everything was outstanding!",
    table: "A4",
  },
  {
    id: "FB-7",
    ...
    ambience: 3,
    value: 2,
    comment: "Naan was cold and hard. Paneer was not fresh. Disappointed.",
    table: "B1",
  },
];
  ...
  Array(5)
    ...
    .map((_, i) => (
      <Star
        key={i}
        size={14}
        color={i < count ? ThemeColors.amber : ThemeColors.border}
        fill={i < count ? ThemeColors.amber : "transparent"}
      />
    ));
...
export default function FeedbackPage() {
  ...
  const avgRating = (
    MOCK_FEEDBACK.reduce((s, f) => s + f.overall, 0) / MOCK_FEEDBACK.length
  ).toFixed(1);
  ...
  const avgFood = (
    MOCK_FEEDBACK.reduce((s, f) => s + f.food, 0) / MOCK_FEEDBACK.length
  ).toFixed(1);
  const avgService = (
    MOCK_FEEDBACK.reduce((s, f) => s + f.service, 0) / MOCK_FEEDBACK.length
  ).toFixed(1);
  const avgAmbience = (
    MOCK_FEEDBACK.reduce((s, f) => s + f.ambience, 0) / MOCK_FEEDBACK.length
  ).toFixed(1);
  const avgValue = (
    MOCK_FEEDBACK.reduce((s, f) => s + f.value, 0) / MOCK_FEEDBACK.length
  ).toFixed(1);
      ...
      >
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}>
            ...
            <Text
              weight="bold"
              style={[styles.statValue, { color: ThemeColors.emerald }]}
            >
              {positiveCount}
            </Text>
          ...
          <View style={styles.statCard}>
            ...
            <Text
              weight="bold"
              style={[styles.statValue, { color: ThemeColors.red }]}
            >
              {negativeCount}
            </Text>
        ...
        <View style={styles.breakdownCard}>
          <Text weight="semibold" style={styles.breakdownTitle}>
            Category Breakdown
          </Text>
          {[
            ...
            <View key={i} style={styles.breakdownRow}>
              ...
              <Text weight="semibold" style={styles.breakdownValue}>
                {cat.value}
              </Text>
            </View>
          ))}
        ...
        <View style={styles.filterRow}>
          {["All", "Positive", "Negative"].map((f) => (
            ...
            >
              <Text
                weight={activeFilter === f ? "semibold" : "regular"}
                style={[
          ...
          <View key={fb.id} style={styles.feedbackCard}>
            <View style={styles.feedbackHeader}>
              ...
              <View style={styles.feedbackMeta}>
                <Text weight="semibold" style={styles.feedbackName}>
                  {fb.customer}
                </Text>
                <Text style={styles.feedbackDate}>
                  {fb.date} • Table {fb.table}
                </Text>
              </View>
