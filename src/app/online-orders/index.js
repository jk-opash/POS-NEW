...
import { updateTableStatus as branchUpdateTableStatus } from "@/store/slices/branchSlice";
import { OnlineOrdersHeader } from "@/components/online-orders/OnlineOrdersHeader";
...
export default function OnlineOrdersScreen() {
  ...
  const addOnlineOrderToKDS = (order) => {
    console.log("Stub: addOnlineOrderToKDS", order);
  };
  
  const updateTableStatus = (id, status) => {
    dispatch(branchUpdateTableStatus({ id, status }));
  };
  
  const injectTableOrder = (id, order) => {
    console.log("Stub: injectTableOrder", id, order);
  };
    ...
    if (newStatus === "Accepted") {
      if (orderToUpdate) {
        ...
        if (orderToUpdate.platform === "QR Order") {
          const tableNameStr = orderToUpdate.customer
            .replace(/table /i, "")
            .trim();
  ...
  return (
    <View style={styles.root}>
      {/* ── Header ──────────────────────────── */}
      <OnlineOrdersHeader isDesktop={isDesktop} newOrderCount={newOrderCount} />
